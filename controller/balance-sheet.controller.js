...
const getCashAndCashEq = (req, res) => {
    ...
    db.getConnection((err, connection) => {
        ...
        const sqlQuery1 = `SELECT franchise_id, balance AS endingBankBalance FROM wafed_bankchecking WHERE uniqueID IN( SELECT MAX(uniqueID) FROM wafed_bankchecking wb INNER JOIN( SELECT franchise_id, MAX(entryDate) entryDate FROM wafed_bankchecking WHERE date_posted > '2015-01-01' AND date_posted <= '${invoiceCreationDate}' GROUP BY franchise_id HAVING franchise_id IN (?) ) wbdp ON wb.franchise_id = wbdp.franchise_id AND wb.entryDate = wbdp.entryDate GROUP BY wb.franchise_id );`;
